...
# get the taxon code for a scientific name 
def get_inaturalist_taxon_code(scientific_name):
    taxon_code = None
    sleep(1)  # To avoid hitting rate limits
    logger.debug("Requesting iNaturalist taxonomy for scientific name =%s", scientific_name)
    
    # Call the iNaturalist API to get the taxon code
    ...
    if response.status_code == 200:
        ...
        if data:
            ...
            if not results:
                logger.warning("No iNaturalist results list for scientific name =%s", scientific_name)
                return None
            
            taxon_code = results[0].get('id')
            logger.debug("Received iNaturalist taxon code=%s for scientific name =%s", taxon_code, scientific_name)
            
        else:
            logger.warning("Empty iNaturalist response body for taxon_code=%s", taxon_code)
            
    else:
        logger.error("iNaturalist API error %s for scientific_name=%s: %s",
                     response.status_code, scientific_name, response.text)
